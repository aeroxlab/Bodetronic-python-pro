...
def visual_distance(a: str, b: str):
    ...
    if av4 and bv4:
        ...
        d_label_hue = _signature_color_distance(av4["label_hue"], bv4["label_hue"])
...
def visual_match_details(a: str, b: str):
    ...
    if av4 and bv4:
        ...
        strict_score = float(os.getenv("VISUAL_STRICT_SCORE", "18"))
        max_label_hue = float(os.getenv("VISUAL_MAX_LABEL_HUE", "18"))
        max_label_text = float(os.getenv("VISUAL_MAX_LABEL_TEXT", "24"))
        max_center = float(os.getenv("VISUAL_MAX_CENTER", "30"))
        max_shape = float(os.getenv("VISUAL_MAX_SHAPE", "32"))
        ...
        safe = bool(score <= strict_score and good_label_color and good_label_text and good_center and good_shape)
...
def detect_product(client_id: int = Form(...), image: UploadFile = File(...), db=Depends(get_db), user=Depends(require_user)):
    ...
    for score, p, ref_url, details in candidates:
        principal_bonus = float(os.getenv("VISUAL_MAIN_IMAGE_BONUS", "6")) if ref_url == p.image_url else 0.0
    ...
    if ranked:
        ...
        principal_ok = bool(principal_scores and min(principal_scores) <= float(os.getenv("VISUAL_PRINCIPAL_MAX_SCORE", "28")))
        very_strict = best_score <= float(os.getenv("VISUAL_VERY_STRICT_SCORE", "13"))
        ...
        accept_product = bool(
            ...
            label_color_ok and label_text_ok and center_ok and shape_ok and
            (
                (best_is_main and (principal_ok or very_strict)) or
                ((not best_is_main) and (very_strict or principal_ok or support_count >= 2))
            )
        )
