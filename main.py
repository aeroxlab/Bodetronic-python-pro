...
def resolve_credit_product(db, client_id: int, payload: dict):
    """Busca producto para fiado.
    1. Usa product_id cuando viene del selector.
    2. Si fue manual, intenta encontrar coincidencia exacta por código de barras,
       código interno, referencia, nombre o nombre visible.
    No hace coincidencias aproximadas para no descontar stock equivocado.
    """
    p = None
    raw_id = payload.get("product_id")
    if raw_id not in (None, "", "null", "undefined"):
        try:
            p = db.query(Product).filter(
                Product.id == int(raw_id),
    ...
    q = norm_text(payload.get("barcode") or payload.get("product_name") or payload.get("name") or "")
    if not q:
        return None

    candidates = db.query(Product).filter(Product.client_id == client_id, Product.active == True).all()
    exact = []
    for prod in candidates:
        values = [
            ...
            prod.reference,
            prod.name,
            product_display_name(prod),
        ]
        if any(norm_text(v) == q for v in values if v):
            exact.append(prod)

    # Solo descontar si hay una coincidencia exacta única.
...
def add_credit_item(payload: dict, db=Depends(get_db), user=Depends(require_user)):
    ...
    # El fiado también usa el mismo inventario.
    # Si viene un producto registrado, o si el texto manual coincide exactamente,
    # se descuenta stock del producto correcto.
    p = resolve_credit_product(db, cc.client_id, payload)
    ...
    if p:
        if qty > float(p.stock or 0):
            return {"ok": False, "error": f"Stock insuficiente. Stock disponible: {p.stock:g}"}
        old_stock = float(p.stock or 0)
        ...
        db.add(StockMovement(
            ...
            product_id=p.id,
            movement_type="DESCARGAR",
            quantity=qty,
            old_stock=old_stock,
            new_stock=new_stock,
            origin="Crédito/Fiado",
            notes=f"Cuenta: {cc.name}"
        ))
    ...
    item = CreditItem(
        ...
        unit_price=unit,
        total=total,
        picked_by=payload.get("picked_by", ""),
        notes=payload.get("notes", "")
    )
    ...
    return {
        "ok": True,
        "item_id": item.id,
        "total_current": cc.total_current,
        "stock_discounted": stock_discounted,
        "product_id": p.id if p else None,
        "old_stock": old_stock,
        "new_stock": new_stock
    }
...
def credit_html_bundle(db, cc: CreditCustomer, items, status="PENDIENTE"):
    ...
    rows = "".join([
        f"""<tr>
        <td data-label="Producto">{i.product_name}</td>
        <td data-label="Cant.">{i.quantity:g}</td>
        <td data-label="P. unit.">{money(i.unit_price)}</td>
        <td data-label="Total">{money(i.total)}</td>
        <td data-label="Fecha">{peru_time_text(i.created_at)}</td>
        </tr>""" for i in items
    ])
    total = sum([i.total for i in items])
    return f"""
    ...
    *{{box-sizing:border-box}}
    body{{font-family:Arial,Helvetica,sans-serif;background:#f4f7fb;margin:0;padding:18px;color:#111827;line-height:1.35}}
    .ticket{{width:100%;max-width:720px;margin:0 auto;background:white;border-radius:22px;padding:24px;box-shadow:0 10px 30px #0001;overflow:hidden}}
    h1{{font-size:30px;line-height:1.08;margin:0 0 22px;word-break:break-word}}
    h2{{font-size:22px;margin:24px 0 8px}}
    p{{font-size:15px}}
    .badge{{display:inline-block;padding:8px 12px;background:#ecfeff;border-radius:999px;font-size:14px}}
    .total{{font-size:36px;color:#00A6A6;font-weight:900;margin:14px 0}}
    .table-wrap{{width:100%;overflow-x:auto}}
    table{{width:100%;border-collapse:collapse;table-layout:auto}}
    td,th{{border-bottom:1px solid #e5e7eb;padding:8px;text-align:left;vertical-align:top;font-size:14px}}
    th{{font-weight:800}}
    @media(max-width:560px){{
      body{{padding:12px;background:#eef3f9}}
      .ticket{{padding:22px 16px;border-radius:22px;max-width:100%}}
      ...
      thead{{display:none}}
      tr{{border:1px solid #e5e7eb;border-radius:16px;margin:12px 0;padding:8px;background:#fff}}
      td{{border:0;border-bottom:1px solid #edf2f7;display:flex;justify-content:space-between;gap:14px;padding:10px 6px;font-size:16px;line-height:1.25;word-break:break-word}}
      td:last-child{{border-bottom:0}}
      td::before{{content:attr(data-label);font-weight:800;color:#111827;min-width:96px;flex:0 0 96px}}
      td[data-label="Producto"]{{font-size:17px;font-weight:700}}
      td[data-label="Producto"]::before{{font-size:16px}}
    }}
    </style></head><body><div class="ticket">
    <h1>{client.business_name if client else 'Bodetronic'}</h1>
    ...
    <div class="table-wrap"><table><thead><tr><th>Producto</th><th>Cant.</th><th>P. unit.</th><th>Total</th><th>Fecha</th></tr></thead><tbody>{rows}</tbody></table></div>
