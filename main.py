...
def visual_match_details(a: str, b: str):
    ...
    Se usa para evitar falsos positivos cuando dos productos tienen forma parecida
    pero diferente color, etiqueta, marca, texto o diseño.
    """
    ...
    if av3 and bv3:
        ...
        max_color = float(os.getenv("VISUAL_MAX_COLOR", "45"))
        max_label = float(os.getenv("VISUAL_MAX_LABEL_TEXT", "52"))
        max_shape = float(os.getenv("VISUAL_MAX_SHAPE", "50"))

        good_color = min(d_color, d_dom) <= max_color
        good_label_text = min(d_label, d_text) <= max_label
        good_shape = min(d_center, d_diff, d_edge) <= max_shape

        # Si el producto se parece solo por forma de envase, no se acepta.
        # Debe concordar también con color/etiqueta/letras o códigos visuales.
        safe = bool(score <= strict_score and good_color and good_label_text and good_shape)

        details.update({
            "avg": round(d_avg, 2),
            "diff": round(d_diff, 2),
            "edge": round(d_edge, 2),
            "color": round(d_color, 2),
            "dominant_color": round(d_dom, 2),
            "center": round(d_center, 2),
            "label": round(d_label, 2),
            "text_like": round(d_text, 2),
    ...
    # Compatibilidad con hashes antiguos: más estricto.
    details["safe_match"] = bool(score <= float(os.getenv("VISUAL_OLD_STRICT_SCORE", "24")))
    return details
...
def detect_product(client_id: int = Form(...), image: UploadFile = File(...), db=Depends(get_db), user=Depends(require_user)):
    ...
    # Compatibilidad con productos antiguos que solo tenían imagen principal.
    for p in db.query(Product).filter(Product.client_id == client_id, Product.active == True).all():
        ...
        details = visual_match_details(h, p_sig)
        d = details.get("score")
        if d is not None:
            candidates.append((d, p, p.image_url, details))

    db.commit()

    # Quitar duplicados por producto, quedándonos con la foto más parecida.
    best_by_product = {}
    for score, p, ref_url, details in candidates:
        if p.id not in best_by_product or score < best_by_product[p.id][0]:
            best_by_product[p.id] = (score, p, ref_url, details)
    ranked = sorted(best_by_product.values(), key=lambda x: x[0])

    if ranked:
        best_score, best_product, best_ref_url, best_details = ranked[0]
        data = product_dict(best_product)
        data["matched_image_url"] = best_ref_url or best_product.image_url

        suggestions = []
        for score, sp, sref, det in ranked[:5]:
            sd = product_dict(sp)
            sd["matched_image_url"] = sref or sp.image_url
            suggestions.append({
                "score": score,
                "confidence": max(0, round(100 - float(score), 2)),
                "safe_match": bool(det.get("safe_match")),
                "details": det,
                "product": sd
            })

        if best_details.get("safe_match"):
            return {
                "ok": True,
        ...
        return {
            ...
            "similar": False,
            "suggestions": suggestions,
            "error": "No hay coincidencia segura. El producto se parece parcialmente, pero no coincide en etiqueta/color/texto/forma."
        }
