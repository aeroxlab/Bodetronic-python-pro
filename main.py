...
DRIVE_UPLOADS = os.getenv('DRIVE_UPLOADS', 'true').strip().lower() in ('1', 'true', 'yes', 'si', 'sí')
...
# ============================
# APPS SCRIPT SYNC + DRIVE BACKUP
# ============================
def drive_upload_bytes(raw: bytes, filename: str, mime_type: str = 'application/octet-stream', folder: str = 'general'):
    """
    Envía archivos al Apps Script para guardarlos en Google Drive.
    Así las fotos/QR/publicidad no dependen del almacenamiento efímero de Render.
    Si falla Drive, el sistema sigue usando el archivo local como respaldo temporal.
    """
    if not DRIVE_UPLOADS or not SHEET_WEBAPP_URL or not raw:
    ...
    try:
        payload = {
            ...
            'filename': filename or ('archivo_' + uuid.uuid4().hex),
            'mime_type': mime_type or 'application/octet-stream',
            'base64': base64.b64encode(raw).decode('ascii')
        }
        r = requests.post(SHEET_WEBAPP_URL, json=payload, timeout=25)
        if 'application/json' in r.headers.get('content-type', ''):
            return r.json()
        return {'ok': r.ok, 'text': r.text[:300]}
    except Exception as e:
        print('[DRIVE_UPLOAD_ERROR]', filename, e)
...
def process_image_upload(upload: UploadFile, folder='products', make_white=True):
    ...
    try:
        ...
        img.save(processed, 'JPEG', quality=90)
        ...
        drive = drive_upload_bytes(processed_bytes, filename, 'image/jpeg', folder)
        final_url = drive.get('direct_url') or drive.get('url') or local_url if isinstance(drive, dict) else local_url

        return final_url, image_hash(img)
    except Exception as e:
...
def upload_file(file: UploadFile = File(...), folder: str = Form('general')):
    ...
    with open(out, 'wb') as f:
        f.write(raw)
    local_url = '/uploads/' + fname
    mime_type = file.content_type or mimetypes.guess_type(fname)[0] or 'application/octet-stream'
    drive = drive_upload_bytes(raw, fname, mime_type, folder)
    final_url = drive.get('direct_url') or drive.get('url') or local_url if isinstance(drive, dict) else local_url
    return {'ok': True, 'url': final_url, 'local_url': local_url, 'drive': drive}
